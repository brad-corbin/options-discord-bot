...
def format_decision_card(
    ...
    dte_rec: Optional[dict],
    v4_result: Optional[dict],
    # Trade details (if trade card was generated)
    spread_type: str = "",
    long_label: str = "",
    short_label: str = "",
    stop_level: float = 0,
    ...
    """
    Compress everything into a 6-line trader decision card.
    """
    direction = bias.get("direction", "NEUTRAL")
    score = bias.get("score", 0)
    conf_label = "?"
    ...
    # Line 1: Direction + confidence
    plain_dir = _plain_direction(direction, score).split("(")[0].strip()
    line1 = f"Bias: {plain_dir.title()}  |  Confidence: {conf_label} ({conf_score:.0%})"

    # Line 2: Range
    line2 = f"Range: ${bear_1sd:.0f} → ${bull_1sd:.0f}  (±${em_1sd:.0f})"

    # Line 3: Dealer pressure (one line)
    dex = eng.get("dex", 0) if eng else 0
    if dex < -0.25:
        line3 = "Dealers short → rallies get buying support"
    elif dex > 0.25:
        line3 = "Dealers long → rallies may face selling"
    else:
        line3 = "Dealer positioning is neutral"

    # Line 4: Environment
    regime_label = regime.get("label", "UNKNOWN")
    if regime_label == "TRENDING":
        line4 = "Moves may accelerate if price starts trending"
    elif regime_label == "SUPPRESSING":
        line4 = "Moves tend to be dampened — range likely"
    elif regime_label == "MIXED":
        line4 = "Mixed signals — breakout possible but not certain"
    else:
        line4 = "Market environment is unclear"

    # Line 5: Best trade
    if spread_type and effective_dte_label:
        line5 = f"Best: {spread_type} ({effective_dte_label})"
    elif dte_rec and dte_rec.get("primary"):
        p = dte_rec["primary"]
        trade_type = "Debit spread" if regime.get("allows_debit_spreads") else "Credit spread"
        line5 = f"Best: {trade_type} ({p['label']})"
    else:
        line5 = "No clear trade setup"

    # Line 6: Execution
    if stop_level and long_label:
        line6 = f"Entry: ~${spot:.0f}  |  Stop: ${stop_level:.0f}  |  Size: {size_pct}%"
    else:
        line6 = f"Entry zone: ${spot:.0f} ± 0.3%"

    lines = [
        f"📊 {ticker} — Decision Card",
        "",
        line1,
        line2,
        line3,
