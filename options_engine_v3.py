...
def build_itm_debit_spreads(
    ...
    """
    v3.6: Adds win probability (from short delta), expected value,
    net delta/gamma per spread, and EM proximity tagging.
    """
    candidates = []
    ...
    # EM lower boundary (for bull spreads, price dropping below this = pain)
    ...
    for long_k in itm_strikes:
        ...
        for width in available_widths:
            ...
            # For ITM bull call spread: P(max profit) ≈ |short call delta|
            # Both legs ITM, so short delta is the weaker leg
            short_delta_abs = abs(short_q.get("delta") or 0)
            win_prob = round(min(short_delta_abs, 0.99), 4) if short_delta_abs > 0 else 0.5

            # ── Expected Value ──
            # EV = (win% × max_profit) - (loss% × max_loss), per contract
            ev = round((win_prob * max_profit) - ((1 - win_prob) * max_loss), 4)

            # ── EM proximity: how far is short strike from EM boundary? ──
            em_proximity = None
            em_zone = "unknown"
            if em_lower > 0:
...
def rank_candidates(candidates: List[Dict]) -> List[Dict]:
    """
    v3.6: Rank by Expected Value instead of pure RoR.

    Ranking score = EV (primary)
                  + width bonus (prefer tighter)
                  + EM zone bonus (short strike inside EM = safer)
                  - liquidity penalty

    This selects trades where (win_prob × profit) - (loss_prob × loss)
    is maximized, with the short strike placed near the probability edge.
    """
    def score(c):
        ...
        # Prefer tighter widths
        width_bonus = 0.3 if width <= 1.0 else 0.1 if width <= 2.5 else 0

        # EM-driven strike placement:
        # Prefer short strike just inside the EM boundary (near the edge)
        # This maximizes probability while keeping reward attractive
        em_bonus = 0
        em_prox = c.get("em_proximity")
        if em_zone == "inside" and em_prox is not None:
            # Closer to EM boundary = better (near the probability edge)
            # em_proximity = 0 means exactly at EM line
            # Small positive = just inside (ideal)
            # Large positive = deep inside (safe but low RoR)
            if em_prox <= 2.0:
                em_bonus = 0.4   # Sweet spot: short strike near EM edge
            elif em_prox <= 5.0:
                em_bonus = 0.2   # Good: inside EM with some buffer
            else:
                em_bonus = 0.05  # Deep inside: very safe but overpriced
        elif em_zone == "outside":
            em_bonus = -0.2      # Short strike beyond EM = riskier

        # Liquidity penalty
        warn_penalty = len(c.get("warnings", [])) * 0.1

        # Combined: EV is normalized to spread width scale,
        # so we weight it heavily but keep other factors meaningful
...
def recommend_trade(
    ...
    # ── Expected Move & IV/RV Edge (v3.6: moved before spread building) ──
    ...
    # ── Build spread candidates (v3.6: EM-aware) ──
    ...
    # (EM, IV/RV edge, vol_edge, em_data already computed above before spread building)
...
def format_trade_card(rec: Dict) -> str:
    ...
    # v3.6 — Win probability + Expected Value
    wp = trade.get("win_prob", 0)
    ev = trade.get("expected_value", 0)
    if wp > 0:
        ev_emoji = "🟢" if ev > 0 else "🔴"
        lines.append(
            f"Win Prob: {wp:.0%} | EV: {ev_emoji} ${ev:.2f}/contract"
        )

    # v3.6 — EM zone placement
    ...
    if em_zone != "unknown" and em_prox is not None:
        ...
        lines.append(
            f"EM Zone: {zone_emoji} Short strike {em_zone} EM "
            f"(${em_prox:+.2f} from boundary)"
        )
    ...
    # Greeks + dynamic exit conditions
    if trade.get("net_theta") is not None:
        delta_str = f"Δ {trade.get('net_delta', 0):.3f}" if trade.get("net_delta") is not None else ""
        gamma_str = f"Γ {trade.get('net_gamma', 0):.4f}" if trade.get("net_gamma") is not None else ""
        theta_str = f"Θ ${trade['net_theta']:.3f}/day"
        vega_str = f"V ${trade.get('net_vega', 0):.3f}/pt"
        parts = [p for p in [delta_str, gamma_str, theta_str, vega_str] if p]
        lines.append(" | ".join(parts))

        # v3.6 — Dynamic exit conditions
        dynamic_exits = []
        nd = trade.get("net_delta")
        ng = trade.get("net_gamma")
        if nd is not None and abs(nd) > 0.85:
            dynamic_exits.append(f"Delta > 0.85 → close early (spread is deep ITM, diminishing returns)")
        if ng is not None and abs(ng) > 0.05 and rec.get("dte", 5) <= 1:
            dynamic_exits.append(f"Gamma spike on 0-1 DTE → tighten stop (pin risk)")
        if trade.get("net_vega") is not None and abs(trade["net_vega"]) > 0.03:
            dynamic_exits.append(f"If IV crushes >5pts → close (vega drag)")

        if dynamic_exits:
            lines.append("⚡ Dynamic Exits:")
            for de in dynamic_exits:
                lines.append(f"  {de}")
