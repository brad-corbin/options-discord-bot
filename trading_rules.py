...
def compute_confidence(
    ...
    Two modes:
      - SIGNAL mode (webhook from Pine): scores based on indicator alignment
      - MANUAL mode (/check command): scores based on trade quality only,
        no penalties for missing indicator data
    """
    reasons = []

    # Detect manual /check mode: no real tier or all indicator values are defaults
    is_manual = (
    ...
    if is_manual:
        # ── MANUAL MODE: /check command ──
        # Base confidence = 55 (you're asking to check it, assume you have a reason)
        score = 55
        ...
        # Liquidity warnings still apply
        ...
        # ── SIGNAL MODE: webhook from Pine Script ──
        score = 30  # base score — signal fired
