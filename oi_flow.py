...
# ── v9 (Patch 6): IV skew threshold recalibration ──────────────────────────
# When STRICT_SKEW_PP=1 in the environment, compute_iv_skew() switches from
# legacy geometric ratio thresholds (skew_extreme = ratio > 1.5) to
# percentage-point thresholds matching options_skew.SKEW_*_PCT. The two
# systems were producing divergent "extreme" calls at low absolute IV — the
# geometric threshold flagged routine 5pp differences as extreme whenever
# absolute IV was compressed (e.g. put_iv 0.15 / call_iv 0.10 → ratio 1.5x
# but only 5pp spread, which options_skew calls STEEP, not EXTREME_STEEP).
# Defaults off (legacy behavior) so deploy is reversible by env var alone —
# no code change required to roll back. The new field skew_pp is always
# returned (forward-compat). Constants below are mirrored from
# options_skew.py — that module remains the source of truth; if those values
# change, this file must be updated to match. See WALK1D_FINDINGS.md.
import os as _os_p6
_STRICT_SKEW_PP = _os_p6.environ.get("STRICT_SKEW_PP", "0") == "1"

# Mirrored from options_skew.py constants (DO NOT EDIT IN ISOLATION).
_SKEW_EXTREME_STEEP_PP = 6.0   # mirrors SKEW_EXTREME_STEEP_PCT
_SKEW_STEEP_PP         = 3.0   # mirrors SKEW_STEEP_PCT
_SKEW_INVERTED_PP      = -0.5  # mirrors SKEW_INVERTED_PCT
...
def compute_iv_skew(chain_data: dict, spot: float) -> dict:
    ...
    Returns: {atm_iv, put_25d_iv, call_25d_iv, skew_ratio, skew_pp,
              skew_direction, skew_extreme}
    - atm_iv: at-the-money implied volatility (decimal)
    - put_25d_iv: ~25 delta put IV (OTM put, decimal)
    - call_25d_iv: ~25 delta call IV (OTM call, decimal)
    - skew_ratio: put_25d_iv / call_25d_iv (geometric)
    - skew_pp: (put_25d_iv − call_25d_iv) × 100 — percentage points (v9 Patch 6)
    - skew_direction: 'put_heavy' | 'call_heavy' | 'neutral'
    - skew_extreme: True if skew is at institutional-fear level

    Threshold semantics depend on STRICT_SKEW_PP env var:
      STRICT_SKEW_PP=0 (default, legacy):        ratio-based
        skew_extreme  ← skew_ratio > 1.5
        put_heavy     ← skew_ratio > 1.2
        call_heavy    ← skew_ratio < 0.8
      STRICT_SKEW_PP=1: percentage-point (matches options_skew.compute_skew)
        skew_extreme  ← skew_pp > 6.0
        put_heavy     ← skew_pp > 3.0
        call_heavy    ← skew_pp < -0.5
    """
    sym_list = chain_data.get("optionSymbol") or []
    n = len(sym_list)
    if n == 0 or spot <= 0:
    ...
    # v9 (Patch 6): skew_pp always computed for forward compat / parity
    # with options_skew.compute_skew.
    skew_pp = (put_25d - call_25d) * 100.0

    if _STRICT_SKEW_PP:
        # v9 (Patch 6): pp-based thresholds, matches options_skew constants.
        skew_extreme = skew_pp > _SKEW_EXTREME_STEEP_PP
        if skew_pp > _SKEW_STEEP_PP:
            skew_direction = "put_heavy"
        elif skew_pp < _SKEW_INVERTED_PP:
            skew_direction = "call_heavy"
        else:
            skew_direction = "neutral"
    else:
        # Legacy geometric thresholds (pre-Patch-6 behavior).
        skew_extreme = skew_ratio > 1.5
        if skew_ratio > 1.2:
            skew_direction = "put_heavy"
        elif skew_ratio < 0.8:
    ...
    return {
        ...
        "skew_pp": round(skew_pp, 2),  # v9 (Patch 6)
