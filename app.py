...
def _find_expiry_for_swing_buffer(ticker):
    try:
        from datetime import date as _date
        today = _date.today()
        exps = get_expirations(ticker)
        ...
        for e in exps:
            ...
            if 28 <= dte <= 35:
                candidates.append((0, abs(dte - 32), dte, e))
            elif 21 <= dte <= 45:
                candidates.append((1, abs(dte - 32), dte, e))
            elif dte >= 15:
                candidates.append((2, abs(dte - 32), dte, e))
        if not candidates:
            return None, None
        candidates.sort()
        _, _, dte, exp = candidates[0]
        return exp, dte
    except Exception as e:
        log.warning(f"_find_expiry_for_swing_buffer failed for {ticker}: {e}")
        return None, None
...
def _post_em_card(ticker: str, session: str):
    try:
        ...
        reason_bits = []
        if bias["direction"] in ("BULLISH", "STRONG BULLISH", "SLIGHT BULLISH"):
            if eng and eng.get("dex", 0) < 0:
                reason_bits.append("dealer hedging supports rallies")
            if flip:
                reason_bits.append(f"holding above gamma flip ${flip:.2f} helps the bullish case")
            if put_wall:
                reason_bits.append(f"put wall near ${put_wall:.2f} is the key support")
        elif bias["direction"] in ("BEARISH", "STRONG BEARISH", "SLIGHT BEARISH"):
            if eng and eng.get("dex", 0) > 0:
                reason_bits.append("dealer positioning can lean against rallies")
            if flip:
                reason_bits.append(f"staying below gamma flip ${flip:.2f} keeps pressure on price")
            if call_wall:
                reason_bits.append(f"call wall near ${call_wall:.2f} is the key resistance")
        else:
            reason_bits.append("signals are mixed, so confirmation matters more than prediction")
            if flip:
                reason_bits.append(f"gamma flip ${flip:.2f} is the main line to watch")

        summary = bias.get("verdict") or "No strong directional edge."
        why_take = "; ".join(reason_bits[:3]) if reason_bits else "defined-risk setups only until price confirms"

        lines += [
            "",
            "Why this matters:",
            f"  {summary}",
            f"  Watch for: {why_take}.",
        ]

        if flip:
            if spot >= flip:
                flip_explain = f"Above ${flip:.2f} now — losing that level can make price action less stable and weaken bullish holds."
            else:
                flip_explain = f"Below ${flip:.2f} now — staying below can amplify moves; reclaiming it can weaken bearish pressure."
            lines += ["", f"Gamma Flip: ${flip:.2f}", f"  {flip_explain}"]

        lines += _format_em_block(em, spot, bias["score"], f"({em_days} calendar days)")

        lines += ["", "Key levels:"]
        if put_wall:
            lines.append(f"  Support / Put Wall: ${put_wall:.2f}")
        if call_wall:
            lines.append(f"  Resistance / Call Wall: ${call_wall:.2f}")
        if gamma_wall:
            lines.append(f"  Gamma Wall: ${gamma_wall:.2f}")
        if not (put_wall or call_wall or gamma_wall):
            lines.append("  No reliable wall data on this chain.")
        ...
        if liquid and eng:
            ...
            lines += [
                "",
                "Data:",
                f"  GEX {tgex:+.1f}M | DEX {dex:+.1f}M | Vanna {vanna_m:+.1f}M | Charm {charm_m:+.1f}M",
            ]
        else:
            lines += ["", "Data:", "  Thin chain — dealer flow data unavailable."]
        ...
        lines.append(f"  PCR: {pcr_oi_s} | {pcr_vol_s}")
        if skew_str:
            lines.append("  " + skew_str.strip())

        lines += ["", f"💡 {iv_note}", "📌 Monitoring only — no trade.", "— Not financial advice —"]
        post_to_telegram("\n".join(lines))
...
def _post_monitor_card(ticker: str, mode: str):
    try:
        ticker = ticker.upper()
        if mode == "long":
            expiry, dte = _find_expiry_for_swing_buffer(ticker)
            mode_label = "Swing Outlook (21-day thesis / buffered DTE)"; mode_emoji = "📅"
        else:
            expiry, dte = _find_expiry_nearest(ticker)
            mode_label = "Near-Term Outlook"; mode_emoji = "⚡"
        ...
        # v4: unified path for both liquid and thin chains
        ...
        if not liquid:
            eng = {}  # strip dealer metrics for thin chains

        if iv is None or spot is None:
            post_to_telegram(f"⚠️ {ticker}: could not fetch IV for {expiry} (DTE={dte})")
            return

        em_days = 21 if mode == "long" else dte
        trading_days = max(em_days * (252/365), 0.5)
        ...
        iv_pct = iv * 100
        if iv_pct < 15: iv_emoji = "🟢"; iv_note = "Low IV"
        elif iv_pct < 30: iv_emoji = "🟡"; iv_note = "Moderate IV"
        elif iv_pct < 50: iv_emoji = "🔴"; iv_note = "Elevated IV — consider spreads"
        else: iv_emoji = "🚨"; iv_note = "EXTREME IV — size down"

        dir_emoji = {"STRONG BULLISH": "🟢🟢", "BULLISH": "🟢", "SLIGHT BULLISH": "🟡",
                     "NEUTRAL": "⚪", "SLIGHT BEARISH": "🟠", "BEARISH": "🔴", "STRONG BEARISH": "🔴🔴"
                    }.get(bias["direction"], "⚪")

        chain_note = "Full institutional analysis" if liquid else "Simplified (thin chain)"
        ...
        if v4_result:
            vr_line = format_vol_regime_line(v4_result)
            if vr_line: lines.append(f"Vol: {vr_line}")
        ...
        if liquid and eng:
            ...
            gex_mode = "SUPPRESSING" if tgex >= 0 else "AMPLIFYING"
            ...
            if v4_result:
                ...
                if ts_line: lines.append(f"  🔍 {ts_line}")

        # ══════ KEY LEVELS ══════
        if walls and ("call_wall" in walls or "put_wall" in walls or "gamma_wall" in walls):
            lines += ["", "─" * 32, "🧱 KEY LEVELS"]
            if "call_wall" in walls:
                cw = walls["call_wall"]; cw_oi = walls["call_wall_oi"]
                dist = ((cw - spot) / spot) * 100
                tag = "inside 1σ" if cw <= em["bull_1sd"] else ("within 2σ" if cw <= em["bull_2sd"] else "outside 2σ")
            ...
            if "put_wall" in walls:
                ...
                tag = "inside 1σ" if pw >= em["bear_1sd"] else ("within 2σ" if pw >= em["bear_2sd"] else "outside 2σ")
                lines.append(f"  🛡️ Support:    ${pw:.2f}  ({pw_oi:,} OI, -{dist:.1f}%) — {tag}")
            if "gamma_wall" in walls and liquid:
                gw = walls["gamma_wall"]; gdir = "above" if gw > spot else "below"
                lines.append(f"  🎯 Gamma Wall: ${gw:.2f}  ({gdir} spot)")
            if "call_top3" in walls:
                lines.append(f"  📵 Resistance: {'  →  '.join(f'${x:.2f}' for x in walls['call_top3'])}")
            if "put_top3" in walls:
                lines.append(f"  🛡️ Support:    {'  →  '.join(f'${x:.2f}' for x in walls['put_top3'])}")

        # ══════ SENTIMENT ══════
        skew_str = _format_skew_line(skew)
        pcr_oi_s = f"OI {pcr['pcr_oi']:.2f}" if pcr and pcr.get("pcr_oi") is not None else "OI n/a"
        pcr_vol_s = f"Vol {pcr['pcr_vol']:.2f}" if pcr and pcr.get("pcr_vol") is not None else "Vol n/a"
        lines += ["", "─" * 32, "📊 SENTIMENT", f"  PCR: {pcr_oi_s}  |  {pcr_vol_s}"]
        ...
        lines += ["═" * 32, "", f"💡 {iv_note}",
                  f"📌 Monitoring only — no trade.", "— Not financial advice —"]

        post_to_telegram("\n".join(lines))
        log.info(f"Monitor card: {ticker} | mode={mode} | DTE={dte} | lean={bias['direction']} | "
                 f"conf={v4_result.get('confidence', {}).get('label', '?') if v4_result else 'N/A'}")
...
def _find_expiry_closest_to_21(ticker):
    try:
        ...
        for e in exps:
            dte = (_date.fromisoformat(e) - today).days
            if dte >= 15: candidates.append((abs(dte - 21), dte, e))
        if not candidates: return None, None
        candidates.sort(); _, dte, exp = candidates[0]
        return exp, dte
    except Exception as e:
...
def _post_monitor_card(ticker: str, mode: str):
    try:
        ...
        if mode == "long":
            ...
            mode_label = "Swing Outlook (≈21 DTE)"; mode_emoji = "📅"
        else:
            expiry, dte = _find_expiry_nearest(ticker)
            mode_label = "Near-Term Outlook"; mode_emoji = "⚡"
