...
SCAN_MAX_DTE = int(os.getenv("SCAN_MAX_DTE", "7") or 7)                         # scan out to this DTE
DEFAULT_MAX_POSTS_PER_SCAN = int(os.getenv("MAX_POSTS_PER_SCAN", "6") or 6)     # cap messages per scan

# Decouple expected move horizon from expiration DTE
...
# Optional: post separate detailed trade message
POST_SEPARATE_TRADE_MESSAGE = (os.getenv("POST_SEPARATE_TRADE_MESSAGE", "0") or "0").strip().lower() in (
    "1", "true", "yes", "y"
)
...
def get_expirations(ticker: str):
    """
    Fetch all expiration dates for a ticker from MarketData.
    Response fields (per docs): s, expirations, updated
    """
    params = {}

    # Optional: request weeklies only if you want
    if (os.getenv("MD_WEEKLY_ONLY", "0") or "0").strip().lower() in ("1", "true", "yes", "y"):
        params["weekly"] = "true"

    data = md_get(f"https://api.marketdata.app/v1/options/expirations/{ticker}/", params=params)

    if not isinstance(data, dict) or data.get("s") != "ok":
        raise RuntimeError(f"Bad expirations response for {ticker}: {str(data)[:180]}")

    exps = data.get("expirations") or []
    if not isinstance(exps, list) or not exps:
        raise RuntimeError(f"No expirations returned for {ticker}: {str(data)[:180]}")
...
def liquidity_warnings_for_trade(contracts, trade):
    ...
    def leg_warn(label, c):
        ...
        if bid is not None and ask is not None and (ask - bid) > LIQ_WARN_BA:
            warnings.append(
                f"Wide bid/ask: {label} ({bid:.2f}/{ask:.2f}) spread {(ask - bid):.2f} > {LIQ_WARN_BA:.2f}"
            )
...
def exp_debug(ticker):
    try:
        ...
        # do NOT 500 here; surface the real error to you
        return jsonify({"error": f"{type(e).__name__}: {str(e)}"}), 200


# TradingView webhook (forces bullish trade suggestion)
...
def tv_webhook():
    ...
    direction = "bull"
    spot = close if close > 0 else get_spot(ticker)
    ...
    try:
        ...
        if isinstance(rec, dict) and rec.get("ok"):
            ...
            # Prefer DEBIT (fix: no indent bugs, no undefined alt)
            alt = None
            ...
            if isinstance(pr, (int, float)):
                if isinstance(ror, (int, float)):
                    trade_lines.append(f"Price: {pr:.2f} | RoR: {ror:.2f}")
                else:
                    trade_lines.append(f"Price: {pr:.2f}")
            else:
        ...
        lines = [
            ...
            (
                f"Walls: Call {call_wall:g} | Put {put_wall:g} | ZeroG {((call_wall + put_wall) / 2):.0f} (est.)"
                if call_wall is not None and put_wall is not None
                else "Walls: —"
            ),
            f"OI Signal: {oi_note}",
...
def scan_watchlist():
    ...
    for ticker in tickers:
        ...
        try:
            ...
            if call_wall is not None and put_wall is not None:
                ...
                direction = "bull" if spot > mid else "bear" if spot < mid else "bull"
    ...
    return jsonify({"status": "ok", "posted": results_posted, "tickers": len(tickers), "debug": debug_lines[:200]})
