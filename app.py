...
# v8.5 (V2 PEER restore): Phase 2.5 peer-mode env vars. Restored from Apr 27
# baseline. Brad runs production with V2_PEER_MODE_ENABLED=1 and
# V2_ORPHAN_POST_MIN_GRADE=B; default stays "0" / "A" so the new behavior is
# strictly opt-in and a forgotten env var deploy keeps legacy behavior.
...
#         * Always CSV-logs to model_comparison_peer_signals.csv with
#           v1_status and card_mode columns so silent disagreements are
#           auditable.
...
# Phase 2.4: suppress untradeable V1 credit vehicles such as $0.03 on $2.50 wide.
V84_CREDIT_SUPPRESS_BAD_VEHICLES = os.getenv("V84_CREDIT_SUPPRESS_BAD_VEHICLES", "1").strip().lower() not in ("0", "false", "no", "off")
    ...
    try:
        ...
        try:
            ...
            if expiry and track_strike > 0:
                ...
                # Register spread tracking for income routes (credit spreads)
                elif route == "income" and track_strike > 0:
                    store = get_option_store()
                    direction = (play.get("trade_direction", "") or "").lower()

                    # Determine spread width based on price level
                    spot = float(play.get("spot", track_strike) or track_strike)
                    if spot < 50:
                        width = 1
                    elif spot < 100:
                        width = 2
                    ...
                    if "bull" in direction:
                        ...
                        spread_side = "put"
                        short_strike = track_strike
                        long_strike = track_strike - width
                        trade_type = "bull_put"
                    else:
                    ...
                    short_occ = build_occ_symbol(ticker, expiry, spread_side, short_strike)
                    long_occ = build_occ_symbol(ticker, expiry, spread_side, long_strike)

                    # Register spread (entry_credit=0 → auto-detect from first streaming quote)
                    store.track_spread(short_occ, long_occ, ticker, trade_type, entry_credit=0)

                    # Ensure both legs are in the streaming subscription
                    try:
                        from schwab_stream import _stream_manager
                        if _stream_manager:
...
def _handle_exit_alert(alert: dict):
    ...
    try:
        ...
        # ── SPREAD EXIT ALERTS ──
        elif alert_type in ("spread_take_profit", "spread_stop_loss"):
            spread_ticker = alert.get("ticker", "?")
            trade_type = alert.get("trade_type", "credit spread")
            entry_credit = alert.get("entry_credit", 0)
            current_value = alert.get("current_value", 0)
            spread_pnl = alert.get("pnl_pct", 0)
            min_value = alert.get("min_value", 0)

            # Parse short/long legs for display
            short_occ = alert.get("short_occ", "")
            long_occ = alert.get("long_occ", "")
            short_parsed = parse_occ_symbol(short_occ) or {}
            long_parsed = parse_occ_symbol(long_occ) or {}
            side_label = (short_parsed.get("side", "") or "").upper()
            spread_label = (f"{spread_ticker} ${short_parsed.get('strike','?')}/"
                           f"${long_parsed.get('strike','?')} {side_label} "
                           f"({trade_type.replace('_',' ').title()})")

            if alert_type == "spread_take_profit":
                msg = (
                    f"💰 SPREAD 50% PROFIT — CLOSE TO LOCK IN\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"Spread: {spread_label}\n"
                    f"Credit received: ${entry_credit:.2f}\n"
                    f"Current value: ${current_value:.2f} (buy to close)\n"
                    f"Profit: ${entry_credit - current_value:.2f}/contract ({spread_pnl:+.0f}%)\n"
                    f"✅ Buy to close — capture 50%+ of max profit"
                )
            else:  # spread_stop_loss
                msg = (
                    f"🛑 SPREAD STOP LOSS — CLOSE TO LIMIT DAMAGE\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"Spread: {spread_label}\n"
                    f"Credit received: ${entry_credit:.2f}\n"
                    f"Current value: ${current_value:.2f} (buy to close)\n"
                    f"Loss: ${current_value - entry_credit:.2f}/contract ({spread_pnl:+.0f}%)\n"
                    f"🛑 Buy to close — spread at 2x credit, cut the loss"
                )
...
def _reconcile_conviction_plays(date_str: str):
    ...
    # Clear conviction + spread tracks after reconciliation
    try:
        from schwab_stream import get_option_store
        get_option_store().clear_conviction_tracks()
        get_option_store().clear_spread_tracks()
    except Exception:
        pass
...
def _flush_wave_digest():
    ...
    for card_text, result in immediate_cards:
        # v8.5 (V2 PEER restore): peer mode inlines V2 as a footer; legacy
        # mode posts V2 as a separate follow-up card. Both branches keep V1
        # untouched. When V2_PEER_MODE_ENABLED is off, behavior is byte-for-
        ...
        if V2_PEER_MODE_ENABLED:
            ...
            if _v2_res is not None:
                ...
                if _spot_for_log is None:
                    _wd_for_log = (result or {}).get("webhook_data")
                    if isinstance(_wd_for_log, dict):
                        _spot_for_log = _wd_for_log.get("close")
                _log_v2_peer_csv(
...
# ══════════════════════════════════════════════════════════════════
# v8.5 (V2 PEER restore): Phase 2.5 peer-mode V2 helpers
# ══════════════════════════════════════════════════════════════════
# Restored from Apr 27 baseline. These were silently deleted between
# Apr 30 → current main while Brad's prod env still had
# V2_PEER_MODE_ENABLED=1, leaving the call sites referencing dead code.
#
    ...
    if job_type in ("tv", "scanner"):
        ...
        if os.getenv("CONVICTION_SCORER_ENABLED", "true").strip().lower() == "true":
            try:
                ...
                # ── v8.5 (V2 PEER restore): Phase 2.5 peer-mode classify ──
                ...
                if _scorer_decision == "discard":
                    # v8.5 (V2 PEER restore): V2 may disagree with the gate —
                    # orphan-post if grade meets threshold (default A).
                ...
                if _scorer_decision == "log_only":
                    # v8.5 (V2 PEER restore): V2 may disagree on score 60-72
                    # signals that V1 quietly dropped — orphan-post if eligible.
                    _maybe_post_v2_orphan(
                        ticker, bias, _v2_result,
                ...
                for cp in _flow_detector.detect_conviction_plays(flow_alerts, dte=max(dte, 0)):
                    try:
                        ...
                        try:
                            ...
                            log.debug(f"Thesis enrichment failed for {cp['ticker']}: {_te}")
                        ...
                        log.warning(f"❌ Conviction play CRASHED for {cp.get('ticker', '?')}: {_cp_err}")
...
def _v84_credit_vehicle_reject_reason(spread: dict) -> str:
    """Return reason when a v8.4 credit spread is too poor to show as tradeable."""
    try:
        width = float((spread or {}).get("width", 0) or 0)
        credit = float((spread or {}).get("credit", 0) or 0)
        roc = float((spread or {}).get("roc_pct", 0) or 0) / 100.0
        if width <= 0 or credit <= 0:
            return "missing width or credit"
        if credit < V84_CREDIT_MIN_CREDIT:
            return f"credit ${credit:.2f} below minimum ${V84_CREDIT_MIN_CREDIT:.2f}"
        if roc < V84_CREDIT_MIN_ROC:
            return f"ROC {roc:.0%} below minimum {V84_CREDIT_MIN_ROC:.0%}"
        return ""
    except Exception as _e:
        return f"quality check error: {_e}"
...
def _post_v84_credit_card(ticker: str, direction: str, spot: float,
    ...
    # the underlying setup is A+. If enabled, post only the V2 review card
    # with vehicle rejected context.
    _v84_reject = _v84_credit_vehicle_reject_reason(spread)
    if V84_CREDIT_SUPPRESS_BAD_VEHICLES and _v84_reject:
        log.info(f"v8.4 CREDIT suppressed for {ticker}: {_v84_reject}")
        if V84_CREDIT_POST_V2_WHEN_SUPPRESSED and V2_5D_EDGE_ENABLED:
            try:
                _supp_v2_result = {
                    "ticker": ticker,
                    "bias": direction,
    ...
    # v8.5 (V2 PEER restore): peer mode — inline V2 footer into card_text BEFORE
    # posting. Compute candidate ranking from the live spread so the footer
    # shows real BE WR / cushion / EV. Only consumed in peer mode; legacy mode
    # below still posts V2 as a separate follow-up card.
    _v2_inline_text = ""
    _v2_best_spread = None
    ...
    # Register spread tracking for 50% TP / 2x credit SL alerts only if Brad
    # explicitly opts into auto-registration. Review cards must not start exit
    # monitoring or behave as if a real entry was taken.
    if REVIEW_CARDS_AUTO_OPEN_ENABLED:
        try:
        ...
        log.info(f"v8.4 credit review-only: skipped spread tracking for {ticker} because REVIEW_CARDS_AUTO_OPEN_ENABLED=0")
            ...
            if _flow_detector:
                ...
                for cp in _flow_detector.detect_conviction_plays(flow_alerts, dte=max(dte, 0)):
                    try:
                        ...
                        log.warning(f"❌ Conviction play CRASHED (site2) for {cp.get('ticker', '?')}: {_cp_err}")
        except Exception:
            pass
            ...
            if _flow_detector:
                ...
                for cp in _flow_detector.detect_conviction_plays(flow_alerts, dte=max(dte, 0)):
                    try:
                        ...
                        log.warning(f"❌ Conviction play CRASHED (site3) for {cp.get('ticker', '?')}: {_cp_err}")
        except Exception:
            pass
