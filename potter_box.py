...
RANGE_EXPANSION_LIMIT = 0.20    # after range seeded, single bar can expand by at most 20%
...
def detect_boxes(bars, ticker):
    ...
    while i < n - MIN_BOX_BARS:
        ...
        body_sum = _body_size(bars[i]); body_count = 1
        j = i + 1
        while j < n:
            bt = _body_top(bars[j]); bb = _body_bot(bars[j]); body = _body_size(bars[j])
            ch = max(running_high, bt); cl = min(running_low, bb)
            mid = (ch + cl) / 2
            if mid <= 0: break
            ...
            if bars_in_box >= MIN_BOX_BARS:
                ...
                if current_range > 0:
                    ...
                    extends_high = bt > running_high
                    extends_low = bb < running_low
                    is_directional = extends_high != extends_low
                    if expansion_ratio > RANGE_EXPANSION_LIMIT:
                        break
                    if is_large and is_directional and range_expansion > current_range * 0.05:
                        break
            running_high = ch; running_low = cl
            body_sum += body; body_count += 1
            j += 1
